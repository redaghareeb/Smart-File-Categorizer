...
class App(ctk.CTk):
    ...
    @staticmethod
    def _detect_llm():
        """Ping local Ollama server to check if it's running and has the right model."""
        import requests
        lang = get_lang()
        result = {"available": False, "display": ""}
        try:
            ...
            if res.status_code == 200:
                ...
                if qwen_models:
                    ...
                    result["display"] = f"✅ AI Core Ready ({qwen_models[0]})" if lang == "en" else f"✅ الذكاء الاصطناعي جاهز ({qwen_models[0]})"
                else:
                    result["display"] = "⚠ Ollama running, but Qwen not found" if lang == "en" else "⚠ المحرك يعمل ولكن نموذج Qwen مفقود"
            else:
                result["display"] = "⚠ AI Core (Ollama) not running" if lang == "en" else "⚠ محرك الذكاء الاصطناعي غير مشغل"
        except Exception:
            result["display"] = "⚠ AI Core (Ollama) not installed or running" if lang == "en" else "⚠ محرك الذكاء الاصطناعي غير مثبت أو متوقف"
        return result
    ...
    def _pg_home(self):
        ...
        llm_status = self._detect_llm()
        status_color = SUCCESS if llm_status["available"] else "gray50"
        
        # Disable switch if AI isn't detected
        if not llm_status["available"]:
            self._llm_var.set(False)
            
        llm_label_txt = "تفعيل محرك الذكاء الاصطناعي (AI/LLM)" if get_lang() == "ar" else "Enable AI/LLM Semantic Engine"
        ctk.CTkSwitch(ai_frame, text=llm_label_txt, font=(FONT,12,"bold"), variable=self._llm_var,
                      state="normal" if llm_status["available"] else "disabled"
                      ).grid(row=0, column=0, padx=16, pady=8, sticky=_sticky())
        
        ctk.CTkLabel(ai_frame, text=llm_status["display"], font=(FONT,11,"bold"), 
                     text_color=status_color).grid(row=0, column=1, padx=16, pady=8, sticky="w" if _rtl() else "e")
        ...
        if engine.CFG.use_llm:
            try:
                ...
                if not any("qwen" in m for m in models):
                    self._show_err("AI Model not found!\nPlease ensure you have installed and run the 'AI Core Setup' (EXE 1).")
                    return
            except Exception:
                self._show_err("AI Brain (Ollama) is not running!\nPlease install 'AI Core Setup' (EXE 1) or start Ollama from your Start Menu.")
                return
